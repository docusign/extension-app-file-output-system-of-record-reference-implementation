 ...
 */
  ...
  updateRecordInFileNonIndex(identifier: string, recordId: string, value: string): void {
    try {
      const jsonArray: object[] = this.readFile();
      // Find a record where any field value matches recordId
      const index = jsonArray.findIndex((record: any) => 
        Object.values(record).some(v => v === recordId)
      );
      if (index === -1) {
        throw new Error(`Unable to find record with field matching parentId: ${recordId}.`);
      }
      (jsonArray[index] as any)[identifier] = value;
      this.writeFile(jsonArray);
    } catch (error) {
      console.error('Error updating file:', error);
      throw error;
